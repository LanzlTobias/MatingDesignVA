...
tmp <- summary_df %>% 
  filter(N == 'N=1000',
         ...
         L == 'L=1000',
         MD == 'HC') %>% 
  ggplot(aes(x=P, y = Mean, fill = ancestral))+
  geom_col(position = 'dodge')+
  facet_grid(~ancestral, scales = 'free_y', switch = 'x')+
...
ggsave('figures/fig2.jpg', width = 7.5, height = 9,
       bg = 'white', units = 'cm')

summary_df %>% 
  filter(N == 'N=1000',
         G == 'G1_DH',
         P %in% paste0('P=', c(2,16)),
         parameter == 'E_Vg',
         L == 'L=1000',
         MD == 'HC') %>% 
  select(P, ancestral, Mean) %>% 
  pivot_wider(values_from = Mean, names_from = P) %>% 
  mutate(increase = `P=16`/`P=2` - 1)
...
summary_df %>% 
  filter(N == 'N=1000',
         ...
         MD == 'HC') %>% 
  select(P, ancestral, Mean) %>% 
  pivot_wider(values_from = Mean, names_from = ancestral) %>% 
  mutate(increase = Landrace/Elite - 1)
```
...
summary_df %>%
  filter(G == 'G1_DH',
         ...
         L == 'L=1000',
         ancestral == 'Elite',
         parameter %in% c('var_b')) %>% 
  select(P, MD, Mean) %>% 
  pivot_wider(names_from = MD, values_from = Mean) %>% 
  mutate(tmp = HC/DC - 1)
...
summary_df %>%
  filter(G == 'G1_DH',
         ...
         parameter %in% c('var_b')) %>% 
  select(P, MD, Mean) %>% 
  pivot_wider(names_from = P, values_from = Mean) %>% 
  mutate(tmp = `P=16`/`P=4` - 1)
...
summary_df %>% 
  filter(G == 'G1_DH',
         parameter == 'var_b') %>% 
  select(L:ancestral, Mean) %>% 
  pivot_wider(names_from = P, values_from = Mean) %>% 
  mutate(tmp = (`P=4` - `P=16`)/`P=4`) %$%
  summary(tmp)
...
summary_df %>%
  filter(G == 'G1_DH',
         ...
         parameter %in% c('var_b')) %>% 
  select(N, Mean) %>% 
  pivot_wider(names_from = N, values_from = Mean) %>% 
  mutate(N50_2_N250 = `N=250`/`N=50` - 1,
         N250_2_N1000 = `N=1000`/`N=250` - 1)
...
summary_df %>%
  filter(G == 'G1_DH',
         ...
         L == 'L=1000',
         MD == 'FC',
         parameter %in% c('var_b', 'var_bb')) %>% 
  select(N, Mean, parameter) %>% 
  pivot_wider(names_from = parameter, values_from = 'Mean') %>% 
  mutate(tmp = var_bb/var_b)
```
...
summary_df %>% 
   ...
   mutate(tmp = G4_DH/G1_DH)
   
summary_df %>% 
    filter(parameter %in% c('var_bw'),
           G %in% c('G1_DH', 'G2_DH')) %>%
  select(L, P, N, MD, ancestral, G, Mean) %>% 
  pivot_wider(names_from = G, values_from = Mean) %>% 
  mutate(tmp = G2_DH/G1_DH - 1) %$%
  summary(tmp)
...
summary_df %>% 
  ...
  pivot_wider(names_from = G, values_from = Mean) %>% 
  mutate(tmp = G2_DH/G1_DH - 1) %>% 
  select(L:ancestral, tmp) %>% 
  pivot_wider(names_from = ancestral, values_from = tmp) %>% 
  mutate(tmp = abs(Elite - Landrace)) %$%
  summary(tmp)
...
summary_df %>% 
  ...
  mutate(tmp = G2_DH/G1_DH - 1) %>% 
  select(L:ancestral, tmp) %>% 
  group_by(L, P, N, ancestral) %>% 
  summarize(tmp = max(tmp) - min(tmp)) %>% 
  ungroup() %$%
  summary(tmp)

summary_df %>% 
    filter(parameter == 'var_b') %>% 
  select(L:G, Mean) %>% 
  pivot_wider(names_from = G, values_from = Mean) %>% 
  mutate(first = G1_DH - G2_DH,
         second = G2_DH - G3_DH,
         third = G3_DH - G4_DH,
         tmp = first > second & first > third) %$%
  table(tmp)

summary_df %>% 
    filter(parameter == 'var_bw') %>% 
  select(L:G, Mean) %>% 
  pivot_wider(names_from = G, values_from = Mean) %>% 
  mutate(first = (G1_DH - G2_DH)/G1_DH,
         second = (G2_DH - G3_DH)/G2_DH,
         third = (G3_DH - G4_DH)/G3_DH) %>% 
  select(L:ancestral, first:third) %>% 
...
summary_df %>% 
  ...
  pivot_wider(names_from = G, values_from = Mean) %>% 
  mutate(first = (G1_DH - G2_DH)/G1_DH) %>% 
  filter(first == min(first) | first == max(first))
...
summary_df %>% 
  ...
  pivot_wider(names_from = G, values_from = Mean) %>% 
  mutate(first = (G1_DH - G2_DH)/G1_DH) %>% 
  ggplot(aes(G1_DH, first, colour = P, shape = N))+
  geom_point()+
...
summary_df %>%
  filter(parameter == 'var_bb',
         G == 'G4_DH') %>% 
  ggplot(aes(x = L, y = Mean, colour = P))+
  geom_point()+
  facet_grid(~N)
...
summary_df %>%
  filter(parameter == 'var_bb',
         ...
         N == 'N=50') %>% 
  select(L:ancestral, Mean) %>% 
  pivot_wider(names_from = L, values_from = Mean)
```
...
ggsave('figures/figS2.jpg', width = 7.5, height = 7.5, bg = 'white', units = 'cm')
```
...
ggsave('figures/figS3.jpg', width = 15, height = 7.5, bg = 'white', units = 'cm')

summary_df %>%
  filter(G == 'G1_DH',
         N %in% c('N=1000'),
         L == 'L=1000',
         parameter %in% c('var_VA', 'var_C')) %>% 
  select(P, ancestral, MD, parameter, Mean) %>% 
  pivot_wider(names_from = parameter, values_from = Mean) %>% 
  mutate(tmp = var_C/var_VA) %$%
  summary(tmp)
...
summary_df %>%
  filter(G == 'G1_DH',
         N %in% c('N=1000'),
         L == 'L=1000',
         parameter %in% c('var_C')) %>% 
  select(N, L, P, ancestral, MD, parameter, Mean) %>% 
  pivot_wider(names_from = ancestral, values_from = Mean) %>% 
  mutate(tmp = Landrace/Elite - 1) %$%
  summary(tmp)
...
ggsave('figures/figS4.jpg', width = 15, height = 12, bg = 'white', units = 'cm')
...
summary_df %>% 
  ...
  select(P, L, Mean) %>% 
  pivot_wider(names_from = L, values_from = Mean) %>% 
  mutate(L50_2_L250 = `L=250`/`L=50` - 1,
         L250_2_L1000 = `L=1000`/`L=250` - 1)

  
summary_df %>% 
  filter(parameter %in% c('var_b', 'var_bb')) %>% 
  select(ancestral, MD, P, N, L, G, parameter, Mean) %>% 
  pivot_wider(names_from = parameter, values_from = Mean) %>% 
  mutate(tmp = var_bb/var_b) %>% 
  select(ancestraL=G, tmp) %>% 
  pivot_wider(names_from = L, values_from = tmp) %>% 
  ggpairs(columns = 6:8)
...
summary_df %>% 
  ...
  select(L:G, Mean) %>% 
  pivot_wider(names_from = L, values_from = Mean) %>% 
  mutate(L1000_250 = (`L=1000` - `L=250`)/`L=1000`,
         L250_50 = (`L=250` - `L=50`)/`L=250`) %$%
  summary(L1000_250)
...
ggsave('figures/figS6.jpg', width = 15, height = 7.5, bg = 'white',
       units = 'cm')
...
D_ancestral %>%
  ...
  ggsave('figures/figS7.jpg', width = 15, height = 15, bg = 'white',
       units = 'cm')
```
