...
p_neg_df <- tibble(data.frame(readRDS(file = 'output/Appendix_A_negative_prob.RData')))
...
```{r}
p_neg_df %>% 
  ggplot(aes(l, neg_prob))+
  geom_point(size = 0.25)+
  geom_hline(yintercept = 1/2)+
  scale_y_continuous(limits = c(0, 1))+
  theme_light(base_size = 10)+
  theme(panel.grid = element_blank())+
  ylab(bquote("P[Y<=y|L," ~ beta ~ "= 0.5]"))+
  xlab('L')

ggsave('figures/figS7.jpg', width = 7.5, height = 7.5, bg = 'white',
       units = 'cm')
```
